...
const TalentMessages = () => {
  ...
  const canSendMessages = ['active', 'trialing'].includes(subscription.status);
  ...
  useEffect(() => {
    const load = async () => {
      const result = await dashboardService.getTalentConversations();
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      const result = await dashboardService.getTalentMessages(selectedId);
      if (result.success && result.data) {
  ...
  const handleSend = async () => {
    ...
    const result = await dashboardService.sendTalentMessage(selectedId, content);
    if (result.success) {
      // Add message locally
      setMessages((prev) => [
        ...prev,
            ...
            {selected ? (
              <>
                ...
                <div className="nex-dash-msg-chat__input">
                  <input
                    type="text"
                    placeholder={canSendMessages ? "Écrivez votre message..." : "Abonnement requis pour envoyer des messages"}
                    value={messageInput}
                ...
                {!canSendMessages && (
                  <UpgradePrompt reason="no_active_subscription" variant="compact" />
                )}
              </>
            ) : (
          ...
          <div className="nex-dash-card__body">
            <div className="nex-dash-empty">
              ...
              <p className="nex-dash-empty__text">
                Vous n'avez pas encore de conversation. Les recruteurs peuvent
                vous contacter depuis votre profil.
              </p>
            </div>
          </div>
        </div>
